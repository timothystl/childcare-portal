...
// ============================================================
// SCHEDULE TIMES  (auto clock-out enforcement)
// ============================================================

function setupScheduleTimes() {
    const el = document.getElementById('schedTimesDate');
    if (el) el.value = new Date().toISOString().split('T')[0];
    document.getElementById('loadSchedTimesBtn')?.addEventListener('click', loadScheduleTimes);
    document.getElementById('saveSchedTimesBtn')?.addEventListener('click', saveScheduleTimes);
    document.getElementById('applyShiftDefaultsBtn')?.addEventListener('click', applyShiftDefaults);
}

async function loadScheduleTimes() {
    const date = document.getElementById('schedTimesDate')?.value;
    if (!date) { alert('Please select a date first.'); return; }

    const container = document.getElementById('schedTimesContent');
    container.innerHTML = '<p class="empty-hint">Loading…</p>';
    try {
        if (!allStaffData.length) await loadStaffList();
        const active = allStaffData.filter(s => s.active);
        if (!active.length) {
            container.innerHTML = '<p class="empty-hint">No active staff members.</p>';
            return;
        }
        const existing = await fetchScheduleTimes(date);
        const schedMap = new Map(existing.map(r => [r.staff_id, r]));
        renderScheduleTimesTable(container, date, active, schedMap);
    } catch (err) {
        container.innerHTML = `<p class="import-error">Failed to load: ${escHtml(err.message)}</p>`;
    }
}

function renderScheduleTimesTable(container, date, staff, schedMap) {
    container.innerHTML = `
        <div class="table-wrapper">
        <table class="report-table">
            ...
            <tbody>
                ${staff.map(s => {
                    const row = schedMap.get(s.id) || {};
                    return `
                        <tr data-staff-id="${s.id}" data-staff-name="${escHtml(s.name)}">
                            <td><strong>${escHtml(s.name)}</strong><br>
                                <span style="font-size:.8em;color:var(--text-muted)">${escHtml(s.role||'')}</span>
                            </td>
                            <td><input type="time" class="sst-start rate-input"
                                value="${row.scheduled_start || ''}"></td>
                            <td><input type="time" class="sst-lunch-out rate-input"
                                value="${row.lunch_out || ''}"></td>
                            <td><input type="time" class="sst-lunch-in rate-input"
                                value="${row.lunch_in || ''}"></td>
                            <td><input type="time" class="sst-end rate-input"
                                value="${row.scheduled_end || ''}"></td>
                        </tr>`;
                }).join('')}
            </tbody>
        </table>
        </div>
        <p class="section-desc" style="margin-top:8px;">* <strong>Shift End</strong> is required to enable auto clock-out enforcement for that staff member. Leave it blank to exempt them for this day.</p>`;
}

async function applyShiftDefaults() {
    const date = document.getElementById('schedTimesDate')?.value;
    if (!date) { alert('Please load a date first.'); return; }
    const container = document.getElementById('schedTimesContent');
    if (!container.querySelector('tbody')) {
        alert('Click Load first to see the staff list.');
        return;
    }

    try {
        const [schedAssignments, defaults] = await Promise.all([
            fetchStaffScheduleWeek(date, date),
            fetchShiftDefaults(),
        ]);

        // Build map: staff_id -> shift ('am'|'pm'|'full')
        const shiftMap = new Map();
        schedAssignments.forEach(row => {
            if (row.work_date !== date) return;
            const existing = shiftMap.get(row.staff_id);
            if (existing === 'am' && row.shift === 'pm') shiftMap.set(row.staff_id, 'full');
            else if (!existing) shiftMap.set(row.staff_id, row.shift || 'full');
        });

        container.querySelectorAll('tbody tr[data-staff-id]').forEach(tr => {
            const staffId = tr.dataset.staffId;
            const shift   = shiftMap.get(staffId) || 'full';
            const def     = defaults[shift] || defaults['full'] || {};
            if (def.start)     tr.querySelector('.sst-start').value     = def.start     || '';
            if (def.lunch_out !== undefined) tr.querySelector('.sst-lunch-out').value = def.lunch_out || '';
            if (def.lunch_in  !== undefined) tr.querySelector('.sst-lunch-in').value  = def.lunch_in  || '';
            if (def.end)       tr.querySelector('.sst-end').value       = def.end       || '';
        });

        const statusEl = document.getElementById('schedTimesStatus');
        if (statusEl) {
            statusEl.textContent = '✓ Defaults applied — review and click Save Times.';
            statusEl.style.color = '#2e7d32';
            setTimeout(() => { statusEl.textContent = ''; }, 4000);
        }
    } catch (err) {
        alert('Failed to apply defaults: ' + err.message);
    }
}
...
async function saveScheduleTimes() {
    ...
    const btn      = document.getElementById('saveSchedTimesBtn');
    const statusEl = document.getElementById('schedTimesStatus');
    btn.disabled    = true;
    btn.textContent = 'Saving…';
    if (statusEl) statusEl.textContent = '';
    ...
    try {
        const rows    = [];
        const cleared = [];
        document.querySelectorAll('#schedTimesContent tbody tr[data-staff-id]').forEach(tr => {
            const staffId = tr.dataset.staffId;
            const start   = tr.querySelector('.sst-start')?.value.trim() || null;
            const lout    = tr.querySelector('.sst-lunch-out')?.value.trim() || null;
            const lin     = tr.querySelector('.sst-lunch-in')?.value.trim() || null;
            const end     = tr.querySelector('.sst-end')?.value.trim() || null;
            if (end) {
                rows.push({ staff_id: staffId, work_date: date,
                    scheduled_start: start, lunch_out: lout, lunch_in: lin, scheduled_end: end });
            } else {
                cleared.push(staffId);
            }
        });

        await Promise.all([
            upsertScheduleTimes(rows),
            deleteScheduleTimes(cleared, date),
        ]);

        if (statusEl) {
            statusEl.textContent = `✓ Saved ${rows.length} schedule(s).`;
            statusEl.style.color = '#2e7d32';
            setTimeout(() => { statusEl.textContent = ''; }, 3000);
        }
        logAdminAction('update', 'staff_schedule_times', date,
            { date, saved: rows.length, cleared: cleared.length });
    } catch (err) {
        if (statusEl) {
            statusEl.textContent = '⚠️ ' + err.message;
            statusEl.style.color = '#c62828';
        }
        console.error('saveScheduleTimes:', err);
    } finally {
        btn.disabled    = false;
...
// ============================================================
// AUTO CLOCK-OUT AUDIT LOG
// ============================================================

function setupAutoClockoutLog() {
    const today   = new Date().toISOString().split('T')[0];
    const weekAgo = new Date(Date.now() - 7 * 86400000).toISOString().split('T')[0];
    const startEl = document.getElementById('aclStartDate');
    const endEl   = document.getElementById('aclEndDate');
    if (startEl) startEl.value = weekAgo;
    if (endEl)   endEl.value   = today;
    document.getElementById('loadAclBtn')?.addEventListener('click', loadAutoClockoutLog);
}

async function loadAutoClockoutLog() {
    const start = document.getElementById('aclStartDate')?.value;
    const end   = document.getElementById('aclEndDate')?.value;
    if (!start || !end) { alert('Select both start and end dates.'); return; }

    const container = document.getElementById('aclContent');
    container.innerHTML = '<p class="empty-hint">Loading…</p>';
    try {
        const rows = await fetchAutoClockoutLog(start, end);
        if (!rows.length) {
            container.innerHTML = '<p class="empty-hint">No auto clock-out events in this range.</p>';
            return;
        }
        const typeLabel = {
            auto_lunch:        'Missed Lunch',
            auto_end_of_shift: 'Missed End-of-Shift',
            auto_hard_cutoff:  '5 PM Cutoff',
        };
        const typeChip = {
            auto_lunch:        'chip-waitlist',
            auto_end_of_shift: 'chip-waitlist',
            auto_hard_cutoff:  'chip-cancelled',
        };
        container.innerHTML = `
            <div class="table-wrapper">
            <table class="report-table">
                <thead><tr>
                    <th>Date</th><th>Staff</th><th>Reason</th>
                    <th>Scheduled</th><th>Auto Clock-Out</th><th>Alert</th>
                </tr></thead>
                <tbody>
                    ${rows.map(r => {
                        const t = new Date(r.actual_clock_out).toLocaleTimeString([],
                            { hour: 'numeric', minute: '2-digit' });
                        return `<tr>
                            <td>${r.work_date}</td>
                            <td><strong>${escHtml(r.staff_name)}</strong></td>
                            <td><span class="status-chip ${typeChip[r.trigger_type]||'chip-confirmed'}">
                                ${typeLabel[r.trigger_type]||r.trigger_type}
                            </span></td>
                            <td>${r.scheduled_time || '—'}</td>
                            <td>${t}</td>
                            <td>${r.alert_sent ? '✅ Sent' : '⏳ Pending'}</td>
                        </tr>`;
                    }).join('')}
                </tbody>
            </table>
            </div>
            <p class="section-desc" style="margin-top:8px;">${rows.length} event${rows.length!==1?'s':''} found.</p>`;
    } catch (err) {
        container.innerHTML = `<p class="import-error">Failed to load: ${escHtml(err.message)}</p>`;
    }
}
