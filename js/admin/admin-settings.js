...
// ============================================================
// CLOCK ENFORCEMENT SETTINGS
// (geofence, director email, shift defaults)
// ============================================================

async function setupClockEnforcement() {
    if (!document.getElementById('clockEnforcementSection')) return;

    // ── Director email ────────────────────────────────────────
    try {
        const email = await fetchDirectorEmailSetting();
        const input = document.getElementById('directorEmailInput');
        if (input && email) input.value = email;
    } catch { /* non-fatal */ }

    document.getElementById('saveDirectorEmailBtn')?.addEventListener('click', async () => {
        const btn      = document.getElementById('saveDirectorEmailBtn');
        const statusEl = document.getElementById('directorEmailStatus');
        const email    = document.getElementById('directorEmailInput')?.value.trim();
        if (!email) { alert('Enter an email address.'); return; }
        btn.disabled = true; btn.textContent = 'Saving…';
        if (statusEl) statusEl.textContent = '';
        try {
            await saveDirectorEmailSetting(email);
    ...
    // ── Geofence ──────────────────────────────────────────────
    try {
        const cfg = await fetchGeofenceSetting();
        const tog = document.getElementById('geofenceEnabledToggle');
        const lat = document.getElementById('geofenceLatInput');
        const lng = document.getElementById('geofenceLngInput');
        const rad = document.getElementById('geofenceRadiusInput');
        if (tog) tog.checked    = cfg.enabled ?? false;
        if (lat) lat.value      = cfg.lat     ?? '';
        if (lng) lng.value      = cfg.lng     ?? '';
        if (rad) rad.value      = cfg.radius_meters ?? 200;
    } catch { /* non-fatal */ }
    ...
    document.getElementById('saveGeofenceBtn')?.addEventListener('click', async () => {
        ...
        if (statusEl) statusEl.textContent = '';
        try {
            const enabled = document.getElementById('geofenceEnabledToggle')?.checked ?? false;
            const lat     = parseFloat(document.getElementById('geofenceLatInput')?.value || '0');
            const lng     = parseFloat(document.getElementById('geofenceLngInput')?.value || '0');
            const radius  = parseInt(document.getElementById('geofenceRadiusInput')?.value || '200', 10);
            if (enabled && (!lat || !lng)) {
                alert('Enter valid latitude and longitude before enabling the geofence.');
                return;
            }
            await saveGeofenceSetting({ enabled, lat, lng, radius_meters: radius });
            if (statusEl) { statusEl.textContent = '✓ Saved'; statusEl.style.color = '#2e7d32'; }
            setTimeout(() => { if (statusEl) statusEl.textContent = ''; }, 3000);
        } catch (err) {
            if (statusEl) { statusEl.textContent = '⚠️ ' + err.message; statusEl.style.color = '#c62828'; }
        } finally { btn.disabled = false; btn.textContent = '💾 Save Geofence'; }
    });
    ...
    async function renderShiftDefaults() {
        const wrap = document.getElementById('shiftDefaultsWrap');
        if (!wrap) return;
        let defaults = {};
        try { defaults = await fetchShiftDefaults(); } catch { /* use empty */ }
            ...
            <table class="report-table" style="max-width:640px;">
                ...
                <tbody>
                    ${SHIFT_KEYS.map(({ key, label }) => {
                        ...
                        return `<tr data-shift-key="${key}">
                            <td><strong>${label}</strong></td>
                            <td><input type="time" class="sd-start rate-input" value="${d.start||''}"></td>
                            <td><input type="time" class="sd-lunch-out rate-input" value="${d.lunch_out||''}"></td>
                            <td><input type="time" class="sd-lunch-in rate-input" value="${d.lunch_in||''}"></td>
                            <td><input type="time" class="sd-end rate-input" value="${d.end||''}"></td>
                        </tr>`;
                    }).join('')}
                </tbody>
            </table>`;
    ...
    await renderShiftDefaults();

    document.getElementById('saveShiftDefaultsBtn')?.addEventListener('click', async () => {
        const btn      = document.getElementById('saveShiftDefaultsBtn');
        const statusEl = document.getElementById('shiftDefaultsStatus');
        btn.disabled = true; btn.textContent = 'Saving…';
        if (statusEl) statusEl.textContent = '';
        try {
            const obj = {};
            document.querySelectorAll('#shiftDefaultsWrap tbody tr[data-shift-key]').forEach(tr => {
                const k = tr.dataset.shiftKey;
                const lout = tr.querySelector('.sd-lunch-out')?.value.trim() || null;
                const lin  = tr.querySelector('.sd-lunch-in')?.value.trim()  || null;
                obj[k] = {
                    start:     tr.querySelector('.sd-start')?.value.trim()     || null,
                    lunch_out: lout,
                    lunch_in:  lin,
                    end:       tr.querySelector('.sd-end')?.value.trim()       || null,
                };
            });
            await saveShiftDefaults(obj);
            if (statusEl) { statusEl.textContent = '✓ Saved'; statusEl.style.color = '#2e7d32'; }
            setTimeout(() => { if (statusEl) statusEl.textContent = ''; }, 3000);
        } catch (err) {
            if (statusEl) { statusEl.textContent = '⚠️ ' + err.message; statusEl.style.color = '#c62828'; }
        } finally { btn.disabled = false; btn.textContent = '💾 Save Defaults'; }
    });
}
