-- Adds half_day to each closure my_schedule() returns to a parent. Purely
-- additive: same signature, same STABLE SECURITY DEFINER, same search_path,
-- same rows readable — one more field in the closures array. Needed so the
-- parent Recap/Schedule tab can label a half-day closure ("Close at 1 pm")
-- distinctly from a full closure instead of just showing "Closed" for both.
